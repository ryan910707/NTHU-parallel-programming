...
// Define the mask as constant device memory
            ...
            for (v = -yBound; v < yBound + adjustY; ++v) {
                for (u = -xBound; u < xBound + adjustX; ++u) {
                    if ((x + u) >= 0 && (x + u) < width && y + v >= 0 && y + v < height) {
                        ...
                        val[i*3+2] += R * d_mask[i][u + xBound][v + yBound];
                        val[i*3+1] += G * d_mask[i][u + xBound][v + yBound];
                        val[i*3+0] += B * d_mask[i][u + xBound][v + yBound];
                    }    
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    sobelKernel<<<gridDim, blockDim>>>(device_s, device_t, height, width, channels);

    // Copy result image to host
